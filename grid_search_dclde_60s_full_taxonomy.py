...
def print_evaluation_report(
    ...
    extra_metrics: Optional[list[tuple[str, Optional[float]]]] = None,
) -> None:
    """Print one readable confusion matrix and per-class metric table."""
    width = max(8, *(len(label) for label in labels))
    print(f"\n{title}")
    print("=" * len(title))
    print(f"Accuracy: {format_metric(overall.get('accuracy'))}")
    print(f"Macro F1: {format_metric(overall.get('macro_f1'))}")
    for label, value in extra_metrics or []:
        print(f"{label}: {format_metric(value)}")

    print("\nConfusion matrix (rows=actual, columns=predicted)")
    print(f"{'actual':>{width}} " + " ".join(f"{label:>{width}}" for label in labels))
    for row_index, actual_label in enumerate(labels):
        values = " ".join(
            f"{int(matrix[row_index, column_index]):>{width}d}"
            for column_index in range(len(labels))
        )
        print(f"{actual_label:>{width}} {values}")

    print("\nPer-class metrics")
    print(
        f"{'label':>{width}} {'support':>8} {'precision':>10} {'recall/TPR':>10} "
        f"{'F1':>10} {'FPR':>10} {'FNR':>10}"
    )
    for row in metric_rows:
        if not row["support"] and not row["predicted_count"]:
            continue
        print(
            f"{row['label']:>{width}} {row['support']:>8d} "
            f"{format_metric(row['precision']):>10} "
            f"{format_metric(row['recall']):>10} "
            f"{format_metric(row['f1']):>10} "
            f"{format_metric(row['false_positive_rate']):>10} "
            f"{format_metric(row['false_negative_rate']):>10}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(print_best_report=True)
    report_group = parser.add_mutually_exclusive_group()
    report_group.add_argument(
        "--print-best-report",
        dest="print_best_report",
        action="store_true",
        help="Print detailed confusion matrices and metrics for the winning grid run (default).",
    )
    report_group.add_argument(
        "--no-print-best-report",
        dest="print_best_report",
        action="store_false",
        help="Suppress the detailed end-of-run console report; CSV reports are still written.",
    )
...
def main() -> int:
    ...
    if args.print_best_report:
        print_evaluation_report(
            "Best configuration: species evaluation",
            species_matrix,
            SPECIES,
            species_overall,
            species_rows,
        ...
        print_evaluation_report(
            "Best configuration: end-to-end KW ecotype evaluation",
            ecotype_matrix,
            ecotype_matrix_labels,
            ecotype_overall,
            ecotype_rows,
            [
                (
                    "Ecotype accuracy conditional on predicted KW",
                    ecotype_overall["conditional_accuracy_when_species_kw"],
                ),
                (
                    "Species-KW recall on ecotype rows",
                    ecotype_overall["species_kw_recall_for_ecotype_rows"],
                ),
            ],
        )
        print_evaluation_report(
            "Best configuration: combined hierarchical evaluation",
            hierarchical_matrix,
            HIERARCHICAL_LABELS,
