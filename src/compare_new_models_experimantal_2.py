...
class MultiSpeciesWindowPredictor:
    ...
    def __init__(
        ...
        print(
            "Multi-species adjacent-window smoothing: "
            f"{'enabled' if self.use_smoothing else 'disabled'}"
        )
    ...
    def _aggregate(self, scores: list[dict[str, float]]) -> tuple[str, float, list[str], list[float]]:
        smoothed_scores = self._smooth_scores(scores) if self.use_smoothing else scores
        local_labels = []
        local_confidences = []
        ...
        for score in smoothed_scores:
            thresholded = {
                ...
                for label, confidence in score.items()
                if float(confidence) >= self.class_thresholds[label]
            }
            if not thresholded:
                label = "other"
                confidence = 0.0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable adjacent-window score smoothing for multispecies aggregation.",
    )
    parser.add_argument(
        "--multispecies-top-k",
        type=int,
